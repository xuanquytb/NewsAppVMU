...
public class AuthApi {
	...
	public ResponseEntity<?> login(@RequestBody @Valid AuthRequest request) {
		try {
			...
			AuthResponse response = new AuthResponse(user.getEmail(), accessToken, true);
	...
	public ResponseEntity<?> auth(@RequestBody @Valid AuthRequest request) {
		try {
			...
			User user = (User) authentication.getPrincipal();
			String accessToken = jwtUtil.generateAccessToken(user);
			AuthResponse response = new AuthResponse(user.getEmail(), user.getNameDisplay(), accessToken, true);
